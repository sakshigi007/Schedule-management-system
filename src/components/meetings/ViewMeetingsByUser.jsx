...
export default function ViewMeetingsByUser() {
  ...
  const getUser = async (id) => {
    const response = await fetch(
      `${process.env.REACT_APP_ENDPOINT_HOST}api/v1/users/${id}`
    );
    const result = await response.json();
    return result;
  };

  const getMeetingsByUser = async (id) => {
    if (id) {
      const response = await fetch(
        `${process.env.REACT_APP_ENDPOINT_HOST}api/v1/schedule/get-meetings/user?userId=${id}`
      );
      const result = await response.json();
      return result;
    }
  ...
  useEffect(() => {
    if (searchParams.get("id")) {
      getUser(searchParams.get("id")).then((result) => {
        if (result.success) {
          setUser(result.data);
          getMeetingsByUser(searchParams.get("id")).then((result) => {
  ...
  return (
    <div className="container">
      <div className="row g-3">
        ...
        {!inProgress && user.userName && (
          <div className="col-12">
            <h4>Meetings by {user.userName}</h4>
          </div>
        )}
        ...
        {!inProgress && meetings.length === 0 && (
          <div className="col-12">
            <h6 className="text-danger">
              No meetings are scheduled for {user.userName}
            </h6>
          </div>
        )}
        {error && (
